...
// sense where a car is located using lidar measurement from point cloud bounding box tranform/center
lmarker Tools::lidarSense1(const Eigen::VectorXf& center, Car& car, pcl::visualization::PCLVisualizer::Ptr& viewer, long long timestamp, bool visualize)
{
	MeasurementPackage meas_package;
	meas_package.sensor_type_ = MeasurementPackage::LASER;
  	meas_package.raw_measurements_ = VectorXd(2);
    
	lmarker marker = lmarker(static_cast<double>(center[0]) + noise(0.15,timestamp), static_cast<double>(center[1]) + noise(0.15,timestamp+1));
	if(visualize)
		viewer->addSphere(pcl::PointXYZ(marker.x,marker.y,3.0),0.4, 0, 1, 1,car.name+"_lmarker");

    meas_package.raw_measurements_ << marker.x, marker.y;
    meas_package.timestamp_ = timestamp;
    //std::cout << meas_package.raw_measurements_[0] << " " << meas_package.raw_measurements_[1] << std::endl;
...
lmarker Tools::lidarSense(Car& car, pcl::visualization::PCLVisualizer::Ptr& viewer, long long timestamp, bool visualize)
{
	...
	if(visualize)
		viewer->addSphere(pcl::PointXYZ(marker.x,marker.y,3.0),0.5, 1, 0, 1,car.name+"_lmarker");
...
{
	UKF ukf = car.ukf;
	viewer->addSphere(pcl::PointXYZ(ukf.x_[0],ukf.x_[1],3.5), 0.5, 1, 1, 1,car.name+"_ukf");
	viewer->addArrow(pcl::PointXYZ(ukf.x_[0], ukf.x_[1],3.5), pcl::PointXYZ(ukf.x_[0]+ukf.x_[2]*cos(ukf.x_[3]),ukf.x_[1]+ukf.x_[2]*sin(ukf.x_[3]),3.5), 0.1, 0.2, 0.6, car.name+"_ukf_vel");
	if(time > 0)
	{
		...
		{
			ukf.Prediction(dt);
			viewer->addSphere(pcl::PointXYZ(ukf.x_[0],ukf.x_[1],3.5), 0.5, 1, 1, 1,car.name+"_ukf"+std::to_string(ct));
...
//Using poin cloud bounding box's center
// Show UKF tracking and also allow showing predicted future path
// double time:: time ahead in the future to predict
// int steps:: how many steps to show between present and time and future time
void Tools::ukfResults1(const Eigen::VectorXf& center, Car car, pcl::visualization::PCLVisualizer::Ptr& viewer, double time, int steps)
{
	UKF ukf = car.ukf;
	viewer->addSphere(pcl::PointXYZ(center[0],center[1],center[2]), 0.5, 1, 0, 0,car.name+"_ukf");
	viewer->addArrow(pcl::PointXYZ(ukf.x_[0], ukf.x_[1],center[2]), pcl::PointXYZ(ukf.x_[0]+ukf.x_[2]*cos(ukf.x_[3]),ukf.x_[1]+ukf.x_[2]*sin(ukf.x_[3]),3.5), 0.1, 0.2, 0.6, car.name+"_ukf_vel");
	if(time > 0)
	{
		double dt = time/steps;
		double ct = dt;
		...
		{
			ukf.Prediction(dt);
			viewer->addSphere(pcl::PointXYZ(ukf.x_[0],ukf.x_[1],center[2]), 0.5, 1, 0, 0,car.name+"_ukf"+std::to_string(ct));
			viewer->setShapeRenderingProperties(pcl::visualization::PCL_VISUALIZER_OPACITY, 1.0-0.8*(ct/time), car.name+"_ukf"+std::to_string(ct));
			//viewer->addArrow(pcl::PointXYZ(ukf.x_[0], ukf.x_[1],3.5), pcl::PointXYZ(ukf.x_[0]+ukf.x_[2]*cos(ukf.x_[3]),ukf.x_[1]+ukf.x_[2]*sin(ukf.x_[3]),3.5), 0, 1, 0, car.name+"_ukf_vel"+std::to_string(ct));
			//viewer->setShapeRenderingProperties(pcl::visualization::PCL_VISUALIZER_OPACITY, 1.0-0.8*(ct/time), car.name+"_ukf_vel"+std::to_string(ct));
			ct += dt;
