...
int main(int argc, char** argv)
{
    ...
    std::vector<boost::filesystem::path> stream = pointProcessorI.streamPcd("../src/sensors/data/pcd");
	...
	double egoVelocity = 55;
    
	rdrfile.open("./Radar_NIS.txt");
	ldrfile.open("./Lidar_NIS.txt");
	while (frame_count < (frame_per_sec*sec_interval))
	{
		...
		highway.stepHighway(egoVelocity,time_us, frame_per_sec, viewer, rdrfile, ldrfile, (*streamIterator).string());
